...
const options = (over: Partial<DocxOptions> = {}): DocxOptions => ({
  header: "BRIDGE360  |  Manual de operador  |  v0.0.1",
  cover: {
    brand: "BRIDGE360",
  ...
  theme: themes.bridge,
  footerNote: "© 2026 Inovisec — Confidencial — Uso Interno",
  vendor: "INOVISEC",
  ...over,
});
...
describe("the cover", () => {
  it("composes from text when no rendered page is supplied", async () => {
    const xml = await documentXml();
    expect(xml).toContain("BRIDGE360");
...
describe("the running header and footer", () => {
  it("carries the brand, the title and the vendor", async () => {
    ...
    expect(header).toContain("BRIDGE360");
    expect(header).toContain("Manual de operador  |  v0.0.1");
    expect(header).toContain("INOVISEC");
  });
  ...
  it("numbers pages with a field, not with text", async () => {
    ...
    expect(footer).toContain("© 2026 Inovisec — Confidencial — Uso Interno");
  });
});
