# Agent Information — `@broadsec-manual/render-docx`
...
and table-splitting rules. Bridge is 73 pages as a PDF and 76 in Word. This is
...
- **Fonts must ship with Office.** Word substitutes a missing family on the
  ...
  because the Bridge faces are frozen — see `font` in `packages/tokens`.
...
```
node packages/cli/src/main.ts build bridge-primera-entrega --docx
```
