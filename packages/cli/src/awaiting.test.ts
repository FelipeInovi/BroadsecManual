...
import type { PendingDeclaration } from "@broadsec-manual/core";
...
const config: ManualConfig = manualConfigSchema.parse({
  manual: { id: "bridge-manual", title: "Manual", product: "Bridge360", contentVersion: "0.4.0" },
  axes: {
    permission: {
...
describe("awaitingProduct", () => {
  ...
  it("counts the sections a queue touches, not just the entries", () => {
    const report = awaitingProduct(config, [
      target("agencia-propia", [decl(), decl({ id: "bot.panel", section: "bridge-of-things" })]),
    ]);
