  ...
  props:
    items:
      - id: filtros.origen-reporte
        ...
        when: { tenant: [mv, demo] }         # per ITEM
```

- Conditioning: **per item.** This is the whole point — a filter only MED has is
...
1. **Conditioning at the item level**, never only at the block level. Broadsec
