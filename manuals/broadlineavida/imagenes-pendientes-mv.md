...
| --- | --- | --- |
| `bot.fig` — Pantalla general del módulo: el panel lateral de navegación y el área de trabajo. | 61 |  |
| `bot.navegacion.fig` — El panel lateral con las secciones del módulo y el estado de las integraciones. | 62 |  |
| `bot.mapa.fig` — La operación sobre el mapa: paneles, vehículos, puntos semaforizados y cámaras. | 63 |  |
| `bot.prt.paso.buscar` — Escriba el criterio de búsqueda | 64 |  |
| `bot.prt.paso.filtrar` — Acote por tipo de vehículo | 65 |  |
| `bot.prt.paso.limpiar` — Restablezca el listado | 65 |  |
| `bot.recursos.paso.ubicar` — Ubique el recurso | 66 |  |
| `bot.recursos.paso.asignar` — Abra la asignación | 66 |  |
...
| `bot.cctv.fn.ptz` — Control PTZ | 69 |  |
...
| `bot.pmv.paso.contenido` — Componga el contenido | 71 |  |
| `bot.pmv.paso.vigencia` — Defina la vigencia | 72 |  |
| `bot.pmv.paso.guardar` — Guarde el mensaje | 72 |  |
...
| `bot.pmv-variables.paso.anadir` — Inicie la creación | 76 |  |
| `bot.pmv-variables.paso.nombrar` — Nombre el grupo | 76 |  |
| `bot.pmv-variables.paso.seleccionar` — Seleccione los paneles | 76 |  |
| `bot.pmv-monitoreo.fig` — El estado de comunicación de cada panel y el mensaje que tiene activo. | 77 |  |
| `bot.pmv-auditoria.fig` — El registro de auditoría, con un asiento por acción realizada. | 77 |  |
| `bot.alarmas.paso.filtrar` — Acote el listado | 79 |  |
| `bot.alarmas.paso.seleccionar` — Seleccione las alarmas | 79 |  |
| `bot.alarmas.paso.reconocer` — Accione Reconocer | 79 |  |
| `bot.alarmas.paso.resultado` — Verifique el resultado | 79 |  |
